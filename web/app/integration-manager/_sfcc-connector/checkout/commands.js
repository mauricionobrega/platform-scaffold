...
const basketUrl = (basketID) => `/baskets/${basketID}`

export const fetchShippingMethodsEstimate = () => (dispatch) => {
    return createBasket()
        .then((basket) => makeApiRequest(`${basketUrl(basket.basket_id)}/shipments/me/shipping_methods`, {method: 'GET'}))
...
export const initCheckoutShippingPage = () => (dispatch) => {
    return createBasket()
        .then((basket) => makeApiRequest(basketUrl(basket.basket_id), {method: 'GET'}))
        .then((response) => response.json())
        .then((responseJSON) => {
            const {
                customer_info: {
